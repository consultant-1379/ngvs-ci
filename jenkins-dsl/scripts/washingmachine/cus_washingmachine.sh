...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/CUS/CUS%20Washingmachine"
  fi
...
MSV="vmx-cus-001"
CIL="vmx-cus-002"
TARGETHOST="vmx-cus-003"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/cus/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/cus/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/cus
