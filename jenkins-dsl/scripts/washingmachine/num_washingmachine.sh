...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/NUM/NUM%20Washingmachine"
  fi
...
INSTALLNODE="vmx-num001"
MSV="vmx-num002"
CIL="vmx-num003"
TARGETHOST="vmx-num004"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/num/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/num/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/num
