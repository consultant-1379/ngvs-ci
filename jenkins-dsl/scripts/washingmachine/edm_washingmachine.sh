...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/EDM/EDM%20Washingmachine"
  fi
...
INSTALLNODE="vmx-edm003"
MSV="vmx-edm004"
CIL="vmx-edm006"
TARGETHOST="vmx-edm007"
TARGETHOST2="vmx-edm012"
EPS="vmx-edm010.ete.ka.sw.ericsson.se:::vmx-edm011.ete.ka.sw.ericsson.se"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/edm/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/edm/washingmachine-${TARGETHOST}-${TARGETHOST2}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/edm
cp $BASE_CONFIG_FILE $CONFIG_FILE
cp $CONFIG_FILE tapas-config.xml

if [[ "${PROCESSOROVFVERSION}" = "LATEST" ]]; then
    PROCESSOROVFVERSION="999.9.9"
fi
if [[ "${EXPOSUREOVFVERSION}" = "LATEST" ]]; then
    EXPOSUREOVFVERSION="999.9.9"
fi
...
--define=__EPS__=${EPS} \
--define=__PROCESSOROVFVERSION__=${PROCESSOROVFVERSION} \
--define=__EXPOSUREOVFVERSION__=${EXPOSUREOVFVERSION} -k killtimeout -v -v -s $CONFIG_FILE >> $logfile 2>&1 &
tapaspid=$!
