...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/CPI/CPI%20Washingmachine"
  fi
...
INSTALLNODE="vmx-cpm-226"
MSV="vmx-cpm-230"
CIL="vmx-cpm-231"
TARGETHOST="vmx-cpm-228"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/cpi/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/cpi/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/cpi
