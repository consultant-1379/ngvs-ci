...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/Taxation/Taxation%20Washingmachine"
  fi
  ...
  # Get jive web url
  jive_web_url="https://jive.epk.ericsson.se/#/projects/taxation/session-groups/session-group/result?context=Washingmachine&suite=JiveSuite&session-user=kascmadm&test-results-filter=%7B%7D&test-results-session-id=$jive_web_session"
...
MSV="vmx-rminvoicing-083"
CIL="vmx-rminvoicing-084"
TARGETHOST="vmx-rminvoicing-085"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/taxation/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/taxation/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/taxation
...
--define=__MSV__=${MSV} --define=__CIL__=${CIL} \
