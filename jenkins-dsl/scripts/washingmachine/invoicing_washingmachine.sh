...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/Invoicing/Invoicing%20Washingmachine"
  fi
  ...
  jive_web_url="https://jive.epk.ericsson.se/#/projects/invoicing/session-groups/session-group/result?context=Washingmachine&suite=JiveSuite&session-user=kascmadm&test-results-filter=%7B%7D&test-results-session-id=$jive_web_session"
...
MSV="vmx-rminvoicing-011"
CIL="vmx-rminvoicing-012"
TARGETHOST="vmx-rminvoicing-013"
TARGETHOST2="vmx-rminvoicing-014"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/invoicing/suites/washingmachine_ovf.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/invoicing/washingmachine_ovf-${TARGETHOST}.xml"
...
--define=__CONTROLLERHOST__=${TARGETHOST} \
--define=__PROCESSORHOST__=${TARGETHOST2} \
