...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/Collection/Collection%20Washingmachine"
  fi
...
INSTALLNODE="vmx-col-001"
MSV="vmx-col-002"
CIL="vmx-col-003"
TARGETHOST="vmx-col-004"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/collection/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/collection/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/collection
...
--define=__STAGINGVERSION__=LATEST -k killtimeout -v -v -s $CONFIG_FILE >> $logfile 2>&1 &
tapaspid=$!
