...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/Finance/Finance%20Washingmachine"
  fi
...
MSV="vmx-fin-002"
CIL="vmx-fin-003"
TARGETHOST="vmx-fin-004"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/finance/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/finance/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/finance
