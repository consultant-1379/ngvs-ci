...
set_description_and_send_blamemail_on_exit(){
  ...
  if [ -z "$tapas_web_url" ]; then
      tapas_web_url="https://tapas.epk.ericsson.se/#/suites/MAPT/MAPT%20Washingmachine"
  fi
  ...
  jive_web_url="https://jive.epk.ericsson.se/#/projects/mapt/session-groups/session-group/result?context=Washingmachine&suite=JiveSuite&session-user=kascmadm&test-results-filter=%7B%7D&test-results-session-id=$jive_web_session"
  echo "jive_web_url: $jive_web_url"
  echo "jive_web_url=$jive_web_url" >> $WORKSPACE/env.properties

  ciscat_result=$(grep 'CIS-CAT Score: ' $WORKSPACE/tapasconsole.txt | cut -d ' ' -f 5-)
...
INSTALLNODE="vmx-mapt-2421"
MSV="vmx-mapt-2424"
TARGETHOST="vmx-mapt-2425"

TAPAS_CONFIG=/proj/eta-automation/config/kascmadm/tapas_production.ini
export TAPAS_CONFIG
TAPAS_BASE="/proj/env/tapas/tapas/bin"

BASE_CONFIG_FILE="/proj/eta-automation/tapas/config/mapt/suites/washingmachine.xml"
CONFIG_FILE="/proj/eta-automation/tapas/sessions/mapt/washingmachine-${TARGETHOST}.xml"
BUILDDESC=`echo ${TARGETHOST}`
export BUILDDESC

mkdir -p /proj/eta-automation/tapas/sessions/mapt
...
echo "WM_BLAME_TIMESTAMP=$WM_BLAME_TIMESTAMP" >> $WORKSPACE/env.properties

${TAPAS_BASE}/tapas_runner.py --define=__INSTALLNODE__=${INSTALLNODE} \
--define=__MSV__=${MSV} \
--define=__TARGETHOST__=${TARGETHOST} -k killtimeout -v -v -s $CONFIG_FILE >> $logfile 2>&1 &
tapaspid=$!
